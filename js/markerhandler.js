  ...
  tick: function () {
    if (elementsArray.length > 1) {
      ...
      if (length === 3) {
        ...
        const m3 = document.querySelector(
          `#marker-${elements[2].barcode_value}`
        );
        const dist1 = this.getDistance(m1, m2);
        const dist2 = this.getDistance(m2, m3);
        if (dist1 < 1.25 && dist2 < 1.25) {
          if (compound !== undefined) {
            const barcodeValue = elementsArray[0].barcode_value;
